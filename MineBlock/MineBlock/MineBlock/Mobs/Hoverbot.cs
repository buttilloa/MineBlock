...
namespace MineBlock.Mobs
{
    public class Hoverbot
    {
        ...
        {
            target1 = Chunk.CalculateChunk(Game1.chunks,(int)Game1.player.highlighted.X, (int)Game1.player.highlighted.Y);
            startx = target1.x;
        ...
        {
            target2 = Chunk.CalculateChunk(Game1.chunks,(int)Game1.player.highlighted.X, (int)Game1.player.highlighted.Y);
            if (mine)
            {
                // target2.MineTime *= 2;
        ...
        {
            target1 = Chunk.CalculateChunk(Game1.chunks,target1.x, target1.y);
                ...
                if (target1.x < target1.x + xcount)
                {
                    if (target1.index != 0)
                    {
                        ...
                        if (minetimer >= minetime)
                        {
                            ...
                            Chunk.PlaceBlock(Game1.chunks,target1.x, target1.y, new Air(0, 0));
                            target1 = Chunk.CalculateChunk(Game1.chunks,target1.x + 1, target1.y);

                        }
                    }
                    else target1 = Chunk.CalculateChunk(Game1.chunks,target1.x + 1, target1.y);
                }
                else { target1 = Chunk.CalculateChunk(Game1.chunks,startx, target1.y + 1); }
            }
            else
            {
                botstate = BotState.follow;
                    ...
                    if (target1.index == 0 && Game1.player.hotbar[Game1.player.selected].Count > 0)
                    {
                        if (dist <= 4)
                        {
                            if (Game1.player.hotbar[Game1.player.selected].Count > 0)
                                Chunk.PlaceBlock(Game1.chunks,target1.x, target1.y,  Game1.player.hotbar[Game1.player.selected].ReturnBlock().Place(target1.x, target1.y));
                            target1 = Chunk.CalculateChunk(Game1.chunks,target1.x + 1, target1.y);
